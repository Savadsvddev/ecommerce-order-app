import { useEffect, useState } from "react";
import ProductItem from "./ProductItem";
import Footer from "../../Layout/Footer";
import { API_BASE_URL } from "../../lib/configuration";
import { getProducts } from "../../api/homeApi";

function ProductList({ selectedCategory }) {

  const staticData = [
    {
      category: "Samsung",
      created_at: "2025-06-19T08:09:30.000Z",
      image: "/images/products/Samsung-S23-Ultra.webp",
      price: "75000",
      product_id: "1",
      product_name: "Samsung-S23-Ultra",
      variants: [
        {
          id: "",
          name: "",
          product_id: "",
          subvariants: [
            {
              id: "",
              package_type: "",
              price: "",
              quantity: "",
            },
          ],
        },
      ],
    },
    {
      category: "Samsung",

      image: "/images/products/Samsung-S25-ultra.jpg",

      price: "50000",
      product_id: "2",
      product_name: "Samsung-S25-Ultra",
    },
    {
      category: "Apple",

      image:"/images/products/Iphone-14.png" ,

      price: "50000",
      product_id: "3",
      product_name: "Iphone-14",
    },
    {
      category: "Apple",

      image:"/images/products/Iphone-15-pro.webp" ,

      price: "60000",
      product_id: "4",
      product_name: "Iphone-15-pro",
    },
    {
      category: "Vivo",

      image: "/images/products/Vivo-V40.png",

      price: "25000",
      product_id: "5",
      product_name: "Vivo-V40",
    },
    {
      category: "Vivo",

      image: "/images/products/Vivo-Y29.webp",

      price: "20000",
      product_id: "6",
      product_name: "Vivo-Y29",
    },
    {
      category: "Oppo",

      image: "/images/products/Oppo-Reno-12.webp",

      price: "30000",
      product_id: "7",
      product_name: "Oppo-Reno-12",
    },
    {
      category: "Oppo",

      image: "/images/products/Oppo-Reno-13.webp",

      price: "35000",
      product_id: "8",
      product_name: "Oppo-Reno-13",
    },
    {
      category: "Realme",

      image: "/images/products/Realme-10.webp",

      price: "15000",
      product_id: "9",
      product_name: "Realme-10",
    },
    {
      category: "Realme",

      image: "/images/products/Realme-12.jpg",

      price: "20000",
      product_id: "10",
      product_name: "Realme-12",
    },
  ];
  const [ProductList, setProductList] = useState(staticData);
  const [loading, setLoading] = useState(false);

  

  // useEffect(() => {
  //   const fetchAllProducts = async () => {
  //     setLoading(true);
  //     let data = {
  //       category: selectedCategory,
  //     };
  //     try {
  //       const response = await getProducts(data);
  //       console.log({ response });
  //       setProductList(response?.data);
  //       setLoading(false);
  //     } catch (error) {
  //       console.error("Failed to fetch orders:", error);
  //       setLoading(false);
  //     }
  //   };

  //   fetchAllProducts();
  // }, [selectedCategory]);

  useEffect(() => {
   if(selectedCategory){
    setProductList(staticData?.filter(ele=> ele?.category === selectedCategory))
   }else{
    setProductList(staticData)
   }
  }, [selectedCategory])
  
  console.log({selectedCategory, staticData})

  return (
    <div className="mt-10">
      <h2 className="text-[#7a1c35] font-bold text-2xl">
        Our Popular Products
      </h2>
      {loading ? (
        <div className="w-full w-100 flex items-center justify-center h-[200px]">
          <div role="status">
            <svg
              aria-hidden="true"
              class="w-10 h-10 text-gray-200 animate-spin dark:text-gray-300 fill-[#7a1c35]"
              viewBox="0 0 100 101"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                fill="currentColor"
              />
              <path
                d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                fill="currentFill"
              />
            </svg>
            <span class="sr-only">Loading...</span>
          </div>
        </div>
      ) : (
        <div
          className="grid gap-5 mt-6
                    grid-cols-2 
                    md:grid-cols-3 
                    lg:grid-cols-4"
        >
          {ProductList?.map((product, index) => (
            <ProductItem product={product} />
          ))}

          {ProductList?.length === 0 ? (
            <h3 className="text-xl">No Products</h3>
          ) : (
            ""
          )}
        </div>
      )}
    </div>
  );
}

export default ProductList;
